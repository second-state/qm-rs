...
**[Getting started with qm-rs →](https://second-state.github.io/qm-rs/)** — set up a team from
nothing: sign in, add people, create a group, and put the agent to work privately and
together.

That guide is *generated by performing it*: `scripts/tutorial.sh` drives a real browser through
every step against a real model, and the screenshots and replies in it are the actual result. If
a step stops working, the guide fails to build.
...
magic-link sign-in and bearer API keys · the directory, participant-keyed groups
and conversational onboarding · Slack (Socket Mode and the signed Events API) ·
