...
* **No channel management UI.** `DirectoryStore` can create channels and add
  members, and the Slack and Telegram connectors do so automatically for the
  rooms they see, but nothing in the web UI or the HTTP API exposes it. A
  web-only deployment therefore has personal scopes and the org scope, and no
  way to make a channel.
